...
<div style="text-align: center; margin-bottom: 30px; padding: 15px; background-color: #f6f8fa; border-radius: 5px; border: 1px solid #e1e4e8;">
  <a href="https://github.com/OwenTat7/NBA_Spread_Prediction_Model" target="_blank" style="text-decoration: none; color: #0366d6; font-size: 18px;">
    <strong>📦 View Project on GitHub</strong>
  </a>
  <br>
  <small style="color: #586069;">https://github.com/OwenTat7/NBA_Spread_Prediction_Model</small>
</div>
...
- **Prediction**: If residual > threshold → Home Covers, else Away Covers
...
3. **Severity Score Calculation**
   ...
   Final Score = Base × Position × Type
   ```
...
**Total**: 18 rolling features (6 metrics × 3 windows)
...
1. **`elo_diff_x_rest_diff`**: ELO difference × Rest difference
...
### Outcome
- ✅ Accurate game dates and times displayed
- ✅ Predictions aligned with actual game schedules
...
### Outcome
- ✅ Optimal threshold: 1.40 (instead of 0.0)
- ✅ Home Cover predictions: 68.9% (down from 91.5%)
- ✅ Test accuracy improved: 58.2% → 61.9%
...
3. **`dk_spread_move`**: Line movement (opening → closing)
...
### Outcome
- ✅ Market information incorporated into model
- ✅ Features show up in top feature importance
- ✅ Helps model understand market expectations
...
### Outcome
- ✅ Injury features represent 9.0% of total feature importance
- ✅ 10 injury features in final model (3 removed due to redundancy)
- ✅ Model captures injury impact on game outcomes
...
1. **Perfect Correlations (r = 1.0)**:
   - `elo_diff` ↔ `elo_spread_estimate`
   - `injury_severity_home` ↔ `total_injury_impact_home`
   - `injury_severity_away` ↔ `total_injury_impact_away`
   - `injury_severity_diff` ↔ `total_injury_impact_diff`
...
### Outcome
- ✅ Reduced from 47 to 43 features
- ✅ Eliminated perfect correlations
...
### Outcome
- ✅ Test accuracy: 61.9% (with optimal threshold)
- ✅ Overfitting reduced: 1.9% gap (excellent generalization)
- ✅ Balanced predictions: 68.9% Home Cover rate
...
### Outcome
- ✅ Optimal threshold: 1.40
- ✅ Test accuracy: 61.9%
- ✅ Home Cover rate: 68.9% (closer to 56.2% actual)
- ✅ Better precision (0.631) and recall (0.774)
...
1. **Fetch Upcoming Games**
   ...
   - Handles timezone conversion (UTC → EST/EDT)
...
- ✅ **61.9% test accuracy** (vs 50% baseline)
- ✅ **Good generalization** (1.9% train/test gap)
- ✅ **Balanced predictions** (68.9% Home Cover rate)
- ✅ **43 well-engineered features**
- ✅ **Comprehensive evaluation** (confusion matrix, metrics)
- ✅ **Production-ready pipeline** (automated, documented)
...
│   ├── predictions_latest.xlsx
│   ├── predictions_latest.csv
│   ├── predictions_history.xlsx
...
│   ├── run_daily_with_logging.sh  # Automation wrapper
│   ├── injury_features.py
│   └── model_diagnostics.py
├── logs/                          # Execution logs
│   ├── daily_predictions_YYYYMMDD.log
│   └── daily_predictions_error_YYYYMMDD.log
├── index.qmd (this file)
├── docs/
└── README.md
```
